...
exports.getAllPropertyFacing = async (req, res) => {
    try {
        ...
        const records = await PropertyFacing.find().skip((pageNumber - 1) * size).limit(size);
...
exports.getPropertyFacingById = async (req, res) => {
    try {
        ...
        if (!propertyFacing) {
            return res.status(404).json({ error: 'PropertyFacing not found',success:false });
        }
        res.status(constants.status_code.header.ok).send({ statusCode: 200, data: propertyFacing,success:true });
    } catch (error) {
...
exports.updatePropertyFacing = async (req, res) => {
    try {
        ...
        const propertyFacing = await PropertyFacing.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!propertyFacing) {
            return res.status(404).json({ error: 'PropertyFacing not found',success:false });
        }
...
exports.deletePropertyFacing = async (req, res) => {
    try {
        const propertyFacing = await PropertyFacing.findByIdAndDelete(req.params.id);
        if (!propertyFacing) {
            return res.status(404).json({ error: 'PropertyFacing not found',success:false });
        }
